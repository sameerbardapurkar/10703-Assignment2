...
class DeepQNetworkDuellingDouble(DQNAgent):
    ...
    def compile(self, loss_func='mse', optimizer=keras.optimizers.Adam()):
        ...
        self.q_network_online.add(Activation('relu'))
        self.q_network_online.add(Dense(self.num_actions + 1))
        ...
        self.q_network_target.add(Activation('relu'))
        self.q_network_target.add(Dense(self.num_actions + 1))
   
        self.q_network_target.compile(loss=loss_func,optimizer=adam)

    def calc_SA_from_q(self, q_vals): 

        sa_func = np.zeros((1, 7))
        val_state = np.max(q_vals)
        adv = q_vals

        for i in range(0, 6):
            adv[0][i] = adv[0][i] - val_state

        sa_func[0][0] = val_state
        for i in range(1,7):
            sa_func[0][i] = adv[0][i-1]

        return sa_func        
        
    def calc_q_from_SA(self, output): 

        q_vals = np.zeros((1, 6))
        val_state = output[0][0]

        # mean_adv = np.sum(output[0][1:6])/6 
        max_adv = np.max(output[0][1:6])

        for i in range(0,6):
            q_vals[0][i] = val_state + (output[0][i+1] - max_adv)
    ...
    def calc_q_values(self, state, is_terminal=False, predict_next_state=False):
        ...
        if(predict_next_state == True):
            output_online = self.q_network_online.predict(self.flatten_for_network(state))
            q_vals_online = self.calc_q_from_SA(output_online)
            index = np.argmax(q_vals_online[0])

            output_target =  self.q_network_target.predict(self.flatten_for_network(state))
            
            q_vals_target = self.calc_q_from_SA(output_target)
    ...
    def fit(self, env, num_iterations = 1000000, max_episode_length=1000):
        ...
        for i in range(0, num_iterations):
            ...
            while(done == False & length < max_episode_length):
                ...
                target_batch_f = np.zeros((len(actions_set), self.num_actions+1))
                ...
                for j in range(0, len(actions_set)):
                    ...
                    target_sa_f = self.calc_SA_from_q(target_f)
                    target_batch_f[j] = (self.flatten_for_network(target_sa_f))
                blah = self.q_network_online.fit(net_current_batch_flat, target_batch_f, batch_size=batch_size_num, epochs=1, verbose=1, callbacks=[keras.callbacks.History()], initial_epoch=0)
