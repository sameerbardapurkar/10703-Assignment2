...
class LinearQNetworkDouble(DQNAgent):
    ...
    def compile(self, loss_func='mse', optimizer=keras.optimizers.Adam()):
        ...
        optimizer. 
        """
        (self.q_network_online).add(keras.layers.Dense(self.num_actions, input_dim=28224, activation = 'linear'))
        (self.q_network_online).compile(loss=loss_func, optimizer=keras.optimizers.Adam())
        (self.q_network_target).add(keras.layers.Dense(self.num_actions, input_dim=28224, activation = 'linear'))
        (self.q_network_target).compile(loss=loss_func, optimizer=keras.optimizers.Adam())
    ...
    def calc_q_values(self, state, predict_next_state=False):
        ...
        if(predict_next_state == True):
            ...
            #prediction = q_vals_target[0][index]
        ...
        if exp_replay:
            for i in range(0, num_iterations):
                ...
                while(done == False & length < max_episode_length):
                    ...
                    blah = self.q_network_online.fit(net_current_batch_flat, target_batch_f, batch_size=batch_size_num, epochs=1, verbose=1, callbacks=[keras.callbacks.History(), keras.callbacks.TensorBoard(log_dir='./logs', histogram_freq=0, write_graph=True, write_images=False)], initial_epoch=0)
            ...
            for i in range(0, num_iterations):
                ...
                while(done == False & length < max_episode_length):
                    ...
                    (output_qvals, prediction1) = self.calc_q_values(net_state_next)
                    (target_f, prediction2) = self.calc_q_values(net_state_current)
                    target_f[0][action] = reward + self.gamma*(output_qvals[0][prediction1])
